...
class Inline:
    ...
    def help_markup(
        self, _lang: dict, back: bool = False, user_id: int = None, is_play: bool = False
    ) -> types.InlineKeyboardMarkup:
        if back:
            callback = "settings_back_helper" if is_play else f"back_{user_id}"
            rows = [
                [
                    self.ikb(text=_lang["back"], callback_data=callback, style=ButtonStyle.PRIMARY),
